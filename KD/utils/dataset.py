...
from monai.transforms import (AddChanneld, Compose, Lambdad, NormalizeIntensityd,RandCoarseShuffled,RandRotated,
                              RandAdjustContrastd,RandGaussianNoised,RandHistogramShiftd,RandCoarseDropoutd,
                              RandZoomd,Resized, ToTensord, LoadImaged, EnsureChannelFirstd)
...
class QaTa(Dataset):
    ...
    def __init__(self, csv_path=None, root_path=None, tokenizer=None, mode='train',image_size=[224,224]):
        ...
        if mode == 'train':
            self.image_list = self.image_list[:int(0.88*len(self.image_list))]
            self.caption_list = self.caption_list[:int(0.88*len(self.caption_list))]
        elif mode == 'valid':
            self.image_list = self.image_list[int(0.88*len(self.image_list)):]
            self.caption_list = self.caption_list[int(0.88*len(self.caption_list)):]
        else:
    ...
    def __getitem__(self, idx):
        ...
        mask_name = self.image_list[idx][: -3] + 'png'  # 解决MoNuSeg数据集图像标签存储的格式不统一的问题
        ...
        return ([image, text], gt, mask_name[: -4])
