...
from .layers_teacher import GuideDecoder
...
class LanGuideMedSeg_teacher(nn.Module):

    def __init__(self, bert_type, vision_type, project_dim=512):

        super(LanGuideMedSeg_teacher, self).__init__()
        ...
        self.out = UnetOutBlock(2, in_channels=24, out_channels=2)

        # deep supervision
        self.convds2 = nn.Conv2d(feature_dim[1], 1, kernel_size=1) # 处理 os16_ds [384, 14, 14]
        self.convds1 = nn.Conv2d(feature_dim[2], 1, kernel_size=1) # 处理 os8_ds [192, 28, 28]
    ...
    def forward(self, data):
        ...
        os16, vis_img_txt16 = self.decoder16(os32,image_features[2], text_embeds[-1])
        os8, vis_img_txt8 = self.decoder8(os16,image_features[1], text_embeds[-1])
        os4, vis_img_txt4 = self.decoder4(os8,image_features[0], text_embeds[-1])
        os4 = rearrange(os4, 'B (H W) C -> B C H W', H=self.spatial_dim[-1],W=self.spatial_dim[-1])

        os8_ds = rearrange(os8, 'B (H W) C -> B C H W', H=self.spatial_dim[-2], W=self.spatial_dim[-2])
        os16_ds = rearrange(os16, 'B (H W) C -> B C H W', H=self.spatial_dim[-3], W=self.spatial_dim[-3])

        out = dict()
        os1 = self.decoder1(os4)
        out['out_logit'] = self.out(os1)
        out['out'] = self.out(os1).softmax(dim=1)
        size = out['out'].shape[2:]
        out['os16_ds'] = F.interpolate(self.convds2(os16_ds), size=size, mode='bilinear', align_corners=False).sigmoid()
        out['os8_ds'] = F.interpolate(self.convds1(os8_ds), size=size, mode='bilinear', align_corners=False).sigmoid()


        return out, vis_img_txt16, vis_img_txt8, vis_img_txt4
