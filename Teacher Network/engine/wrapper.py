...
class LanGuideMedSegWrapper(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(), lr = self.lr) #lr = self.hparams.lr
    ...
    def shared_step(self,batch,batch_idx):
        x, y = batch
        outputs, *alignment_teacher = self.model(x)
        loss = self.loss_fn(outputs['out_logit'],y)


        return {'loss': loss, 'preds': outputs['out'].detach(), 'y': y.detach()} # 传递给‘preds’用于Dice函数计算指标时，logit需经过softmax，这里的实现是正确的
    ...
    def shared_step_end(self,outputs,stage):
        ...
        for name in metrics:
            step_metric = metrics[name](outputs['preds'][:,1,:,:].unsqueeze(dim=1), outputs['y']).item()
            if stage=="train":
