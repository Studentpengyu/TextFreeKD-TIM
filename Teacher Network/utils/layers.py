...
class GuideDecoderLayer(nn.Module):
    ...
    def forward(self,x,txt):
        ...
        # Cross-Attention-1
        vis2_align = self.norm2(vis_img)
        vis2, w1 = self.cross_attn1(query=self.vis_pos(vis2_align),
                                   key=self.txt_pos(txt),
                                   value=txt) # 交叉注意力输出的维度主要由query决定。
        vis2 = self.cross_attn_norm1(vis2)
        vis_img_txt = vis_img + self.scale1*vis2  # 引入文本信息的图像特征，将这个特征蒸馏到vis_img
...
class GuideDecoder(nn.Module):
    ...
    def forward(self, vis, skip_vis, txt):
        ...
        if txt is not None:
            vis_img_txt = self.guide_layer(vis, txt)

        vis = rearrange(vis_img_txt,'B (H W) C -> B C H W',H=self.spatial_size,W=self.spatial_size)
